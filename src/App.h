 ...
 * application is waiting to start. Starting is initializing the prerequisite condition to
 ...
 * but will not remove the parameter. Application will finish all task and save all
...
{
  char text[30];
  sprintf(text, "ID: %d -- ", ID_Application);
  Serial.print(text);
  Serial.print(Name_Application);
  Serial.print(" : ");
  Serial.println(State_Application_String());
}
#endif